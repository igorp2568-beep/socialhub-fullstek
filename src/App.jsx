...
import { LayoutDashboard, Plus, List, CheckCircle, XCircle, Settings, Trash2, RefreshCw, Clock, Send, Image, Film, Bell, Check, X, Loader2, Eye, Upload, User, ChevronLeft, ChevronRight, Sparkles, Webhook, Database, AlertCircle, Zap, ExternalLink } from "lucide-react";
...
const AI_PROVIDERS = {
  openrouter: { name:"OpenRouter (multi-modelos gratuitos)", url:"https://openrouter.ai/api/v1/chat/completions", models:["meta-llama/llama-3.3-8b-instruct:free","google/gemini-2.0-flash-exp:free","deepseek/deepseek-r1:free","qwen/qwen3-8b:free"], placeholder:"sk-or-v1-...", link:"openrouter.ai/keys" },
  openai: { name:"OpenAI — ChatGPT", url:"https://api.openai.com/v1/chat/completions", models:["gpt-4o-mini","gpt-4o","gpt-4-turbo","o1-mini"], placeholder:"sk-...", link:"platform.openai.com/api-keys" },
  ...
  google: { name:"Google — Gemini", url:"__google__", models:["gemini-2.0-flash","gemini-2.0-flash-thinking-exp","gemini-1.5-flash","gemini-1.5-pro"], placeholder:"AIzaSy...", link:"aistudio.google.com/app/apikey" },
  groq: { name:"Groq — ultra rapido", url:"https://api.groq.com/openai/v1/chat/completions", models:["llama-3.3-70b-versatile","llama-3.1-8b-instant","gemma2-9b-it"], placeholder:"gsk_...", link:"console.groq.com/keys" },
  ...
  zhipu: { name:"Zhipu AI / GLM (China)", url:"https://open.bigmodel.cn/api/paas/v4/chat/completions", models:["glm-4-flash","glm-4","glm-4-plus"], placeholder:"sua-api-key", link:"open.bigmodel.cn" },
  doubao: { name:"ByteDance Doubao (China)", url:"https://ark.cn-beijing.volces.com/api/v3/chat/completions", models:["doubao-lite-4k","doubao-pro-4k","doubao-pro-32k"], placeholder:"seu-endpoint-id", link:"console.volcengine.com/ark" },
  minimax: { name:"MiniMax (China)", url:"https://api.minimax.chat/v1/text/chatcompletion_pro", models:["abab6.5s-chat","abab6.5-chat"], placeholder:"sua-api-key", link:"api.minimax.chat" },
  mistral: { name:"Mistral AI (Europa)", url:"https://api.mistral.ai/v1/chat/completions", models:["mistral-small-latest","mistral-large-latest","open-mistral-nemo"], placeholder:"sua-api-key", link:"console.mistral.ai" },
  cohere: { name:"Cohere Command", url:"__cohere__", models:["command-r-plus","command-r","command"], placeholder:"sua-api-key", link:"dashboard.cohere.com" },
...
const aiGenerate = async (context, cfg) => {
  ...
  const prompt = (cfg.ai_prompt?cfg.ai_prompt+"\n\n":"")+"Gere uma legenda criativa para Instagram sobre: \""+context+"\". Inclua emojis e hashtags. Responda APENAS com a legenda pronta.";
  ...
  const d = await r.json(); if(d.error) throw new Error(typeof d.error==="string"?d.error:d.error?.message||"Erro"); return d.choices?.[0]?.message?.content;
};
...
const makeApi = () => {
  ...
  return {
    ...
    async uploadFile(file) {
      ...
      if(!r.ok) throw new Error("Upload falhou - verifique bucket posts no Supabase");
...
function MiniCalendar({ selectedDate, onChange }) {
  ...
  return (
    <div style={{...neu.inset,borderRadius:20,padding:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,marginBottom:6}}>{["D","S","T","Q","Q","S","S"].map((d,i)=><span key={i} style={{textAlign:"center",fontSize:10,color:C.muted,fontWeight:600}}>{d}</span>)}</div>
...
function FileDropzone({ onFile, file, uploading }) {
  ...
  return (
    <div onClick={()=>ref.current.click()} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);handle(e.dataTransfer.files[0]);}}
      style={{...neu.inset,padding:28,textAlign:"center",cursor:"pointer",borderRadius:20,border:drag?"2px dashed "+C.primary:"2px dashed rgba(255,255,255,0.08)"}}>
      <input ref={ref} type="file" accept="image/*,video/*" style={{display:"none"}} onChange={e=>handle(e.target.files[0])}/>
      {uploading?(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}><Loader2 size={32} style={{color:C.primary,animation:"spin 1s linear infinite"}}/><span style={{color:C.muted,fontSize:14}}>Enviando para Supabase...</span></div>)
      ...
      :(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
        <div style={{...neu.flat,width:60,height:60,borderRadius:18,display:"flex",alignItems:"center",justifyContent:"center"}}><Upload size={26} style={{color:C.muted}}/></div>
  ...
  return (
    <div style={{...neu.inset,borderRadius:14,padding:14}}>
      ...
      <div style={{display:"flex",gap:8}}>
        <input value={ctx} onChange={e=>setCtx(e.target.value)} placeholder="Descreva o conteudo (ex: foto K-pop Seoul, paisagem Japao...)" style={{flex:1,...neu.inset,padding:"10px 12px",color:C.onSurface,fontSize:13,outline:"none",background:"#12192b",border:"1px solid rgba(255,255,255,0.08)"}} onKeyDown={e=>e.key==="Enter"&&gen()}/>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:24,maxWidth:720}}>
      ...
      {tab==="contas"&&(<div style={{display:"flex",flexDirection:"column",gap:20}}>
        <div style={{...neu.flat,padding:28}}>
          ...
          <button onClick={() => {
            ...
            const scope = "instagram_business_content_publish,instagram_business_manage_messages";
        ...
        <div style={{...neu.flat,padding:24}}>
          ...
          <p style={{fontSize:12,color:C.muted,margin:"0 0 14px"}}>Ja tem o token e o ID?</p>
      ...
      {tab==="ia"&&(<div style={{...neu.flat,padding:28,display:"flex",flexDirection:"column",gap:20}}>
        ...
        <div><label style={lbl}>Provedor</label>
        ...
        {provider==="custom"&&<div><label style={lbl}>URL da API</label><input style={inp} placeholder="https://sua-api.com/v1/chat/completions" value={cfg.ai_custom_url||""} onChange={e=>setCfg(c=>({...c,ai_custom_url:e.target.value}))}/></div>}
        ...
        <div><label style={lbl}>Instrucoes Personalizadas</label><textarea style={{...inp,minHeight:70,resize:"vertical"}} placeholder="Ex: Sempre use tom divertido. Foco em K-pop." value={cfg.ai_prompt||""} onChange={e=>setCfg(c=>({...c,ai_prompt:e.target.value}))}/></div>
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:28}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:20}}>
      <div style={{...neu.flat,overflow:"hidden"}}>
        ...
        {loading?<div style={{display:"flex",justifyContent:"center",padding:48}}><Loader2 size={26} style={{color:C.primary,animation:"spin 1s linear infinite"}}/></div>:posts.length===0?(<div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"48px 32px",gap:16}}>
